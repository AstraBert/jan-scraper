...
async def on_message(message):
    if message.author == bot.user:
        ...
        if len(queue) == 0:
            ...
            c.close()
            response = scrape_jan_through_api(str(message.content),model="llama2-chat-7b-q4",new_instructions="You are a wellbeing assistant focused on giving your best advice and psychological and emotional support to make people feel respected and accepted for who they are")
            c = open("queue.csv","r+")
            lines = c.readlines()
            c.seek(0)
            c.truncate()
            ...
            c.write(f"{job_time},{str(job_author)}\n")
            c.close()
            response = scrape_jan_through_api(str(message.content),model="llama2-chat-7b-q4",new_instructions="You are a wellbeing assistant focused on making people feeling comfortable and respected at every moment")
            c = open("queue.csv","r+")
            lines = c.readlines()
            c.seek(0)
            c.truncate()
